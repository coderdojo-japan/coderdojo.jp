...
INPUT_TEXT.each do |line|
  ...
  found_dojo = DOJO_DB.find do |dojo|
    dojo[:name] == dojo_name.downcase
      ...
      .gsub('町田',           'まちだ')
      .gsub('小平',           'こだいら')
      .gsub('八戸@吹上', '八戸').gsub('八戸', '八戸@吹上')  # 「八戸」「八戸@吹上」どちらの入力でもOK（二重正規化を防ぐ）
      .gsub('八尾@yotteco',   '八尾')
      .gsub('平野',           '平野@YOZORA LABO')
      .gsub('麹町@アドバンスト・ソフト', '麹町').gsub('麹町', '麹町@アドバンスト・ソフト')  # 同上
      .gsub('港北nt',         '港北NT')          # Google Forms 経由の「港北NT」入力に対応（downcase 対策）
      .gsub('港北ニュータウン', '港北NT')          # 旧表記「港北ニュータウン」も同じ道場へ寄せる
      .gsub('吉備okayama',    '吉備')
      .gsub('吉備岡山',       '吉備')
      .gsub('愛媛新居浜',     '新居浜')
      .gsub('名護 恩納@oist', '名護')
