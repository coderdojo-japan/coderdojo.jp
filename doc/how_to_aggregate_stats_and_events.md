...
それでも実行する場合は、開始日を明示して許可してください:
  ALLOW_DESTRUCTIVE_REAGGREGATION=2012-01-01 rails 'statistics:aggregation[2012-01-01,2026-08-24]'
```

環境変数の値には**対象期間の開始日そのもの**を要求している。`1` のような固定値だと、
シェルに `export` が残ったまま別の実行まで意図せず承認してしまうため。
